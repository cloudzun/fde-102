...
| 选型的两个层次 | 流程层（设计期定死，承载边界与验收）/ 节点层（多数交给模型） | 第 11 章 11.3.7 |
| 受控 Agent | 动态选工具、观察后继续、追问、真暂停（实操四B） | 第 11 章 11.3.8 |
| 敏感件零漏判 | "给定验证集召回率 100%，生产目标零漏判"两步口径 | 第 11 章 11.7.3、第 12 章 |
| 工具工程化五件套 | 超时 / 重试 / 幂等 / 审计 / 降级 | 第 11 章 11.6.3 |
| 数据安全域 | 数据可合法处理的组织批准环境；六类"出域"口径 | 第 7 章 7.4 |
| 价值证据链 | 痛点→基线→能力→技术指标→业务变化→可量化价值 | 第 15 章 15.2 |
| 五类价值口径 | 年度价值（效率 + 质量 + 收入增益 + 风险避免）/ 年净收益 / TCO / ROI / 回收期（不混用） | 第 15 章 15.2.2 |
| 收益成本比 | 累计收益 / 总投入（"赚回几倍"的说法，≠ ROI） | 第 15 章 15.2.2 |
