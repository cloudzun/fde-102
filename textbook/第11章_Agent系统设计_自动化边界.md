...
> **说明：** 实操四B 从头构建独立项目、不复制第 12 章代码，但复用本章的 LangGraph、工具设计和 HITL 概念（手册见资源包 `optional-labs/`，不要求全员必做）。判断"该用工作流还是 Agentic Loop"的方法就是二维判断：**路径可枚举 → 路由工作流；路径不可枚举或需反复观察 → 受控 Agent**。
