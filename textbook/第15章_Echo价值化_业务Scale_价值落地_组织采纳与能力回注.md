...
一年"多得到"了什么——省时、少错、避险。这是"赚了多少"的总账，回答"这一年值多少钱"。

**年净收益 = 年度价值 − 年运营增量成本**

从年度价值里扣掉为维持它而新增的运营成本（模型调用、算力、人工复核）——"赚的"减去"为赚而新增的运营开销"，才是真正落袋的。

**TCO（总拥有成本）= 建设成本 + 运行成本 + 维护成本 + 人工审核成本**

比"一次性采购价"更能反映政务长期负担：采购只是开始，三年运维加人工审核才是大头——决策层问"总共要花多少"，回答的是这个。

**ROI（投资回报率）=（累计收益 − 总投入）/ 总投入**

净收益相对投入的**回报率**，不是"倍数"；客户若想要"赚回几倍"的说法，另用 **收益成本比 = 累计收益 / 总投入**，两者不要混用。注意 ROI 必须覆盖**整个投资周期**（累计收益对累计投入），不是一年的数字。
...
| 价值类型 | 年价值公式 | 例子 |
|---------|-----------|------|
| **效率型**（省时间） | 日均处理量 × **年运行天数** × AI 实际覆盖比例 × 平均每单节省工时 × 单位时间成本 | 日均 2000 条 × 250 天 × 60% 简单件 × 每条省 1.5 分钟 × 时薪 40 元 |
| **质量型**（少出错） | 日均处理量 × **年运行天数** × 原错误率 × AI 后错误率降幅 × 单次错误损失 | 日均 200 笔（年 5 万）× 250 天 × 漏检率 2%→0.3% × 单次损失 5000 元 |
| **收入型**（多赚钱） | 日均增量转化量 × **年运行天数** × 客单价 × 净利率 | 日均多接待 27 咨询 × 250 天 × 转化 10% × 客单价 2000 元 × 净利率 30% |
...
> **对决策层的话术：** 报给客户："最可能年省 32 万；即使效果打折（保守）也有十几万——可验证的经济可行性，不是拍脑袋。"再用"**上线后 N 周用真实运营数据出真数字**"收尾，而不是承诺单点。
...
flowchart TB
    ...
    FIX["未过：补短板<br/>门槛动机信任 / 口径 / 生产差距 / 容量"]
...
```
*图 15-2：业务 Scale 试点—推广流程——Gate 全过才扩大，不过补短板回验证，持续不达标就暂停/停止*
...
| 检查项 | 通过证据 | 未通过时的动作 |
|--------|---------|--------------|
| 采用 | 种子用户持续使用，试点范围内采用率达到目标 | 回"组织采用"一节补门槛/动机/信任，不扩大 |
| 价值 | 业务指标复核与价值证据链一致，无夸大 | 修正价值口径或缩小范围继续验证 |
| 工程 | 运行稳定、告警闭环、客户团队能接管（第 14 章） | 不进入扩大范围，先补生产差距 |
| 容量 | 人工审核容量跟得上转人工量 | 调整自动化边界或增加人力 |

> **Echo 的表达必须建立在 Delta 已提供的工程证据上**——"能稳定运行"说成"跨组接管演练通过"、"不会漏"说成"给定验证集召回率 100%"（第 11 章口径），并给出 15.3.3 的监控指标（转人工率、拒答率、类别分布）作为前哨。
...
- **五口径分开**：年度价值 / 年净收益 / TCO / ROI / 回收期（精确定义见 15.2.2）；只算 AI 真覆盖部分、给区间不给单点、每个数字标来源、书面锁口径（估算与话术见 15.2.3）。
- **组织采用三关**：门槛（简化+种子用户）→ 动机（角色升级+参与验收标准）→ 信任（《谁负责》+试点真数据+回滚），每关都指向第 14 章证据（15.3.1）。
- **四类干系人表达**：决策层听价值、操作层听负担、技术层听接管、监管层听合规（15.3.2）。
- **业务 Scale**：试点 → 真实数据复核 → 扩大范围 Gate → 停止条件（15.3.3）。
...
- [ ] 我能为第 16 章备好"业务证据"（价值 / 采用 / 回注 / 风险四块）吗？
