...
> **闭环程度递进：** 三个单点 MVP 各自闭合"一次动作"，实操四B（Lab4B）闭合"一条动态轨迹"（选工具 → 观察 → 追问 / 暂停 → 收束或转人工）。第 16 章对两者**分别验收**（第一层三场景、第二层实操四B）；有代码基础的学员可用实操四B 做综合毕业项目，继承三类闭环能力并新增多轮状态、追问与真暂停（进阶路径见 7.6 的实操四B 纪律与资源包 `optional-labs/`）。
