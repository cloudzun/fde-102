...
> **适配说明：** 沿用训练营"实操三"骨架；聚焦这一轮的**新决策点**（RAG 特有），复用实操二已有流程。
...
- [ ] 政策文档素材在手（Lab3-policy-docs 目录，5 份混合格式）
- [ ] 创建本实操目录 `实操三-政策问答/`
...
> `.docx` 和 `.pdf` 要先做**格式解析**（转成可检索文本）再分块向量化。政策文档是 Lab3-policy-docs 目录下的真实素材，**施工时让 Agent 从该目录读取，不让 Agent 自己编政策内容**。
...
**Step 1 写出 7 项信息 + 政策文档路径 + Q&A 验证集（见附录 B）。** 
**Step 2 用自查问题审查**（同实操二 7 个自查问题）。
**Step 3 附上 Q&A 验证集（6 条，见附录 B）**，让 Agent 落盘 `tests/qa_pairs.csv`。
...
注入：把启动提示词灌进 `/office-hours` + 附驱动指令（同实操二，价格件落盘、禁止全自动、拍板点列选项）。
...
- **本体贡献**：建立"诉求—政策条款—政策文件"的依据关系（业务本体的输入，见第 15 章 15.5）。
...
- **资源包：** 政策文档（Lab3-policy-docs）、Q&A 验证集、启动提示词参考答案（随书资源包 / 训练营合订本同源）。
...
（同第 8 章附录 A：清华镜像、ghfast 代理、克隆 + setup、验证 skills。）

## 附录 B · Q&A 验证集与启动提示词参考答案

**Q&A 验证集（6 条，让 Agent 落盘 `tests/qa_pairs.csv`）：**
```
...
```
> expected_source 统一用**解析后文件名（.md）** + 条款号，验收系统引用时才能严格对上。

**启动提示词参考答案核心段（供环节 1 完成后对照）：**
```
【背景】上一轮完成场景 A（诉求分类），陈主任催着推进场景 B（政务政策问答），已选型 RAG。
【要做什么】政务政策知识问答系统。
【输入输出】输入：政策问题；输出：回答 + 来源引用（哪份文件、哪一条）。
【技术选型】RAG。不用微调（政策常更新需溯源易幻觉）、不用纯关键词搜索（语义鸿沟）。
【数据现状】政策库 Lab3-policy-docs/（5 份混合格式）+ Q&A 验证集（tests/qa_pairs.csv）。
  政策会更新，需要索引重建机制。
【政策文档】Lab3-policy-docs/ 共 5 份：三个 .md + 户籍迁移政策.docx + 就业创业补贴政策.pdf。
  对 .docx/.pdf 先格式解析（转可检索文本），内容不得自行改动或编造。
【验收口径】双指标：答案准确率 ≥80% + 来源可追溯率 ≥90%。来源错误（引用错文件/条款）算未达标。
【约束】答案必须带来源引用；检索不到明确说"政策库中未找到，建议咨询人工"，绝不编造；
  API key 走环境变量。
```
> **三处"防翻车"：** ①"不用微调/关键词搜索"（防走错路线）；②"来源错误算未达标"（防只优化答案不重视引用）；③"检索不到必须明说不能编"（防编造政策，政务底线）。
