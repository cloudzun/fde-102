...
> **本章定位：** 实操单元 · **角色 = Delta** · Prototype 阶段 · 模式 = **AI 施工**（opencode + gstack）· **场景 B**——Delta 侧第二个施工实操（坡度第二级：检索 + 生成）。
>
> **前置：** 场景 B 方案框架；RAG 链路与选型；施工工作流；实操二成果。
>
> **延伸阅读：** 见文末 **FDE-101** 章节方法深读链接。
>
...
|---|---|---|---|
| 复用实操二施工工作流 | 演练流程全程 | 启动提示词含 7 项信息 | 含政策文档 + Q&A 验证集 |
| 施工 RAG（解析→分块→向量化→检索→生成+引用） | 环节 2 注入并跟进八环节 | 索引与检索代码、qa_pairs.csv | 5 份文档读全、建索引抽查 |
| 双指标验收落地 | 环节 3 验收与收尾 | qa-report.md 双指标结果 | 双指标达标、拒答实测 |
| RAG 特有决策点（切块/top-K/兜底） | 环节 2 注入并跟进八环节 | SPEC-决策记录.md | 决策点都拍板 |
| 演示级验收声明 + 交接 | 环节 3 验收与收尾 + 环节 4 复盘沉淀 | 验收声明、retro.md | 声明与交接完成 |

## 10.1 知识背景

> **这一轮的方法定位：**
> - **开卷考试**：本实操落在四层级金字塔第 2 层 RAG——先检索政策片段，再基于片段作答并标注来源；不用微调（政策常更新、需溯源、易幻觉）、不用纯关键词搜索（语义鸿沟）。
> - **核心链路**：离线建库（解析→分块→向量化→存库）做一次，在线问答（检索→生成 + 引用）做多次；政策更新只需重建索引，不用改模型。
> - **双指标**：答案准确率 + 来源可追溯率，是政务 AI 的准入门槛——答必须带出处、检索不到不能编。
> - **从最简单开始**：本实操用基础向量检索（Chroma + top-K），不堆叠混合检索 / 重排；当评估发现某类查询持续差，才逐段升级。
> - **数据不出域**：本实操 embedding 走 API（如硅基流动 bge-m3）的前提，是**课堂模拟数据（完全虚构、脱敏、无真实个人信息）可在课堂批准环境使用公网服务**；若换成真实客户数据（未经确认不得进入公网服务），应切到本地向量库与本地 embedding。
> - **最小工具契约（供后续复用）**：本实操的检索能力将被封装为 **`search_policy`** 工具——**输入**：用户问题；**输出**：带来源的证据片段列表。每个证据带**来源元数据**：`source_file`（原始文件）/ `normalized_file`（归一化文本）/ `article`（条款）/ `excerpt`（摘要），供"来源可追溯"与实操四B 复用，并守住"无证据不回答"的底线。元数据字段现在建好，后面建索引、答引用、做本体都不用返工。

## 10.2 任务书

> 打开解决方案框架的场景 B："政务政策知识问答——让市民问政策，系统直接给答案，还得告诉他是哪份文件、哪条规定的，免得坐席背锅。"
>
...
> **这不是让 AI 干的活，是你（Delta）的活。** 流程和实操二一样，但你这次有了经验，且场景 B 的 7 项信息内容不同。对照方案框架场景 B 填：
...
>
> **验证集设计：验证集问题不要设计得太宽。** 一个宽问题往往覆盖多个条款（如"外地户口能交社保吗"同时涉及稳定就业/灵活就业两条），单条款锚定必然歧义。**拆细**（稳定就业→第15条、灵活就业→第16条）能同时提升验收稳定性和教学清晰度。

### 10.4.2 环节 2 · 注入并跟进八环节
...
>
> **统计提示（小样本口径）：** 6 条验证集上，"可追溯率 ≥90%" 实际需要 **6/6 全对**——5/6≈83.3% 不达标；小样本下 90% 门槛**退化为"全对制"**。该结果**只说明本次样本**，不代表稳定的生产水平；对"全对才过线"要有心理预期。
...
## 10.6 Echo Prototype 检查单

> 本章不是 Delta 单线活动：Prototype 结束时，**Echo 至少完成一次检查**（对照实操一的方案框架），产出可归因、可评分的个人记录。Delta 仍是施工主责。

Echo 至少完成：

- 哪些问题允许回答、哪些必须拒答（回答边界确认）；
- 引用是否真正支撑结论（抽查"答对但引错/引对但过度推断"两类失败）；
- 政策适用对象、时间和地域边界确认；
- 坐席是否能理解和复核答案（对小王演示后的反馈）；
- 是否满足原始业务需求的判断。
...
|---|---|---|---|---|---|---|
| 场景 B 双指标口径 | RAG + 引用 + 拒答 | qa-report.md | 演示反馈 | 待填 | 是否调整及原因 | Go / 继续 / 回炉 |
...
> [!IMPORTANT]
> **红线本处落地：答案可追溯。** 本章你亲手把"答案必须带来源 + 检索不到不能编 + 双指标验收"写进 SPEC 与 DoD——这是政务问答类项目常见的验收准入门槛，也是导读页四条红线的本处闭环。
...
- **本体贡献**：建立"诉求—政策条款—政策文件"的依据关系（业务本体的输入）。
...
- **挑战（迁移练习/升级路径）：** 想对齐完整版 RAG 工程可按需升级——1）**混合检索 + 重排序**（bge-m3 hybrid + bge-reranker-v2-m3，召回 top-100→top-10）；2）**RAGAS 三维评估**（忠实度/上下文精度/回答相关性替代/补充双指标）；3）**生产化**（ACL 元数据、增量更新而非全量重建）。说明每项该在"什么时候发现问题再上"，而非一开始堆砌。
...
- **实验手册：** 政策文档（`labs/lab3-policy-docs/`）、Q&A 验证集、启动提示词参考答案见《实验手册》`labs/lab3-policy-rag.md`。
- **下一章：** Agent 方法 · 自动化边界（坡度第三级：决策循环 + 自动化边界）。
