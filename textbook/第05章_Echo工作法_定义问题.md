...
> **选层是 Echo 的"方向"决策；选定之后"怎么建出来"由 Delta 用 AI Coding 承接（见第 7 章 7.2–7.5：建层、gstack、四条纪律）。金字塔是 Echo → Delta 的交接接口——双方对着同一张图分工：Echo 选层、Delta 建层。**
...
**顺带收集"候选业务语义"（不做最终建模）：** 访谈与跟班时记下四类观察，作为后续本体建模的输入（第 15 章 15.4 展开）：
...
> [!NOTE]
> Discovery 阶段**只收集候选语义，不完成最终本体**——对象边界、关系与动作的正确性要等数据与流程验证后再定（第 14 章 14.9 工程验证、第 15 章 15.4 建模）。
...
《解决方案框架》不只是"做什么"，还要写清"做到什么程度算成"——每个场景填一张八字段表，它决定后续每个 Delta 实操（第 8/10/12 章）的验收口径从哪来：
...
> **实践边界：** Ch6 实操一（求判断力）和 Ch16 实操五（求表达力）**关键判断不用 AI**——训练的是 FDE 最不可替代的判断力；真实项目里当然可用 AI 提效，但那是在你已经具备判断力之后。
