...
>
> **本章产出：** 你能说出"Discovery 初期不以功能开发为目标"的原则、原因与**时间盒技术验证**的边界；熟练使用三个思维脚手架（失败模式 / 决策链 / 能力金字塔）；独立完成三个 SOP（干系人地图 / 数据资产盘点 / 快赢筛选），每个 SOP 都知道**输入 / 完成判据 / 异常应对**；完成"场景拆解 + 选型三问 + 技术决策矩阵"；能独立产出一页纸《解决方案框架》；用 **Discovery Gate** 六要素核验自己能否进入 Prototype；了解 AI 如何辅助 Discovery。
>
> **建议读者：** 偏 Echo 者精读，偏 Delta 者也需理解（要在施工时看懂 Echo 的方案框架怎么来的）。
>
> **前置：** 第 3 章（Discovery 在四阶段中的位置）、第 4 章（Echo 角色）。
...
- **前置知识：** 第 3 章（四阶段）、第 4 章（Echo 角色）。
- **章节地图：** 本章是理论，**紧接着的实操一（西岭需求调研）马上用**——两者是"知识 → 动手"的一对；Gate 总纲在本章落到 Discovery 的可操作核验。
...
为什么不是先动手？因为 **People 是根、Process 是脉、Technology 是器——顺序不可颠倒。** 一上来就奔着"做出功能"去，你会在没搞清"客户到底要解决什么、谁在用、数据行不行"之前，就把方案做在错误的地基上。
...
Echo 在 Discovery 初期的时间分配，大致这样分配仅供参考：

```mermaid
pie title Echo 在 Discovery 初期的时间分配
    "听客户说话·干系人访谈" : 35
    "跟班观察业务现场" : 25
    "数据资产盘点" : 20
    "验证与方案设计（含时间盒验证）" : 20
...
> **这也是与 Delta 的根本分工：** Echo 的第一任务是把问题"定义对"，而不是"动手做"。判断错再快的施工也是白费。

---

## 5.2 三个思维脚手架

头脑风暴 / 诊断不是"随便聊聊"，而是用三个脚手架让讨论有结构、有产出。它们是 Echo 的核心知识。
...
**用它的方法：** 对每个子场景问"该用哪一层、为什么是它、为什么不是更复杂的那层"。**新手最容易"什么都想上 Agent"，要拉回"从最简单开始"。**（金字塔不是"单线升级"路线，各维度可组合。）

> **选层是 Echo 的"方向"决策；选定之后"怎么建出来"由 Delta 用 AI Coding 承接。金字塔是 Echo → Delta 的交接接口——双方对着同一张图分工：Echo 选层、Delta 建层。**
...
- **示例（西岭）**：决策层陈主任（高权力·高兴趣）、监管层市数据局（高权力·低兴趣）、操作层坐席组长小王（低权力·高兴趣）、技术层信息中心李工（低权力·低兴趣/中性）。
...
**顺带收集"候选业务语义"（不做最终建模）：** 访谈与跟班时记下四类观察，作为后续本体建模的输入：
...
> [!NOTE]
> Discovery 阶段**只收集候选语义，不完成最终本体**——对象边界、关系与动作的正确性要等数据与流程验证后再定。
...
- **输入**：已拆解的子场景清单（有初稿即可）；每个场景"业务需要什么数据"的问题清单。
...
> - **场景 C 工单分流** → 工作流（预编排三路路由 + 工具调用 + 敏感件转人工 HITL）——不是分类器，因为分流后还要"做事"（自动答复/转人工）。
...
离开 Discovery 进入 Prototype 前，按 Discovery Gate 做六要素核验：

| Gate 要素 | 本阶段内容 |
|---|---|
| **输入条件** | 已立项 / 客户授权；可接触干系人与数据入口 |
| **核心活动** | 干系人地图、痛点诊断、数据盘点、快赢筛选、场景拆解与选型 |
...
《解决方案框架》不只是"做什么"，还要写清"做到什么程度算成"——每个场景填一张八字段表，它决定后续每个 Delta 实操的验收口径从哪来：
...
> [!IMPORTANT]
> **"教学初始值"与"校准"分开写。** 教材若给默认阈值（如准确率 ≥85%），必须标注为**教学初始值**，不得声称"已由学员确定"；Prototype 结束后允许基于证据校准阈值，但**必须记录变更原因与确认人**（校准记录进入第 16 章证据包）。Delta 不得自行发明业务阈值——那是 Echo 的职责。

> **这份《解决方案框架》就是 Echo 交给 Delta 的"施工图/任务书"**——后续 Delta 施工照着它做。
>
> **完整成品样例不随正文公布**——它是实操一的产出，公布出来等于直接给答案；讲师可在实操一验收后组织讲评对标。
...
> **实践边界：** 实操一（求判断力）和 Ch16 实操五（求表达力）**关键判断不用 AI**——训练的是 FDE 最不可替代的判断力；真实项目里当然可用 AI 提效，但那是在你已经具备判断力之后。
...
- **把"不以功能开发为目标"误解为"完全不碰技术"。** 时间盒技术验证是被鼓励的——它的产出是"判断依据"，不是"可用功能"。
- **跳过"先独立、后碰撞"。** 头脑风暴一上来就开口讨论，会被声量大的人带偏；先每人独立写判断，才能暴露真实分歧（实操一专门练）。
- **把"拍脑袋的数字"当依据。** 基线的"现状数据"（如错分率）若没统计，是整个 ROI 方案的地基——**哪些数字必须先核实，要列清楚**；快赢评分每项都必须附依据，不允许"凭感觉"打分。
- **SOP 做完没有判据。** 干系人地图画了、数据盘了，却说不出"这步算没算完成"——每个 SOP 必须过一遍完成判据。
- **对决策层讲技术黑话。** Echo 汇报要用客户听得懂的业务语言，能让陈主任直接拿去市里汇报。
- **把"高风险高复杂度"当场就上。** 什么都想上 Agent / 微调——从最简单开始（能力金字塔 + 决策树 + 决策矩阵）。

> 🔺 **本书红线（首次在方法层落地）：数据不出域** 是政务 AI 的关键选型约束——Echo 在 Discovery 就必须把它作为设计约束写入方案（《解决方案框架》第五节），而非施工期才发现。本条红线在实操一将亲手落地。
...
- 产出《解决方案框架》（场景+选型+优先级+风险+数据不出域）——即 Delta 的施工图；正文不公布成品样例（避免直接给出实操一的答案）。
...
- **挑战（迁移练习）：** 挑一个你熟悉的真实模糊需求（一句话），用本章工具拆成子场景并做选型，产出一页纸《解决方案框架》初稿，并用 Discovery Gate 六要素逐项自检——指出你缺哪条通过证据、打算怎么补。

---

## 延伸阅读

- 想读 Discovery 的完整 SOP 与工作底稿、访谈提纲、数据清单模板，见 **FDE-101** https://www.cloudzun.com/fde-course/（第 5 章 Phase 1 Discovery、第 16 章 AI FDE 落地：Delta 技术与场景）。
