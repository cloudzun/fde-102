...
- **两种训练模式（本书设计）**：Echo 侧判断型实操（Ch6/Ch16）**关键判断不用 AI**，AI 仅可辅助整理/润色/扮演红队——练判断力；Delta 侧实操用 **opencode + gstack（AI 施工）** 练施工力。
...
- **本书对应**：第 16 章实操五演练"MVP 达标 ≠ 生产可用"的差距评估，第 14 章做模拟客户环境的完整接入演练（本书以评估 + 模拟演练为主，完整生产 Build 在真实项目中进行）。
...
> **本 Gate 的业务侧证据在第 15 章**（价值证据链 15.2、组织采用 15.3、回注联合决策 15.4），**工程侧证据在第 14 章 14.9**（业务本体工程验证与工程回注），**综合评审在第 16 章实操五**（Go / Conditional Go / Continue Pilot / No-Go）。
...
> [!NOTE]
> **图 3-6 是全书首发总览**，呈现"能力层级与交付复杂度"的全貌。该金字塔后续会从不同视角复用：**第 5 章图 5-2** 给它一层"Echo 业务视角"（标注各层的业务适用边界与成本/风险权重，回答"该不该用这层"）；**第 7 章图 7-2** 再给一层"Delta 工程视角"（标注各层的实现形态与工程代价，回答"选定之后怎么建"）。读后面那两张图时，可回看本图保持整体框架。
...
- 政企场景的第一约束（本书案例红线，也是常见政企约束，**非普遍法定义务**）是**数据不出域**：核心数据不能上公有云，必须本地/私有化部署——具体边界由数据分类分级、监管要求与合同确定（六类"出域"口径见 13.2）（第 1 章五大鸿沟之一；第 6 章实操一把它写进方案框架）。
...
> 详细展开见 **第 15 章 15.4（能力回注的联合决策：业务通用性 × 工程复用性）**；第 14 章 14.9 给出"业务本体的工程验证"。
