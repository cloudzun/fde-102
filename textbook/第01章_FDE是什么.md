...
>
> **需要澄清的一点：** 三位一体是"FDE 团队"的整体能力画像，而非要求每个 FDE 个人都是全能选手。本书按**教材口径**将其落地为前线作战单元的分工：Echo（业务策略）+ Delta（技术执行）+ Engineering（平台保障）——Echo 负责业务顾问维度、Delta 负责工程师执行、Engineering 负责产品抽象与平台保障。详见第 4 章分工。
...
> **业务语义的起点：** FDE 的价值不仅是连接数据，还要把客户的**业务对象、关系和可执行动作**建成统一模型——这正是能力回注中"业务语义层"的雏形（第 3 章 3.5 总览、第 15 章 15.4 展开）。
...
- FDE 模式的早期实践通常归因于 Palantir：**把工程师带核心平台驻场、做深度定制部署、再把通用能力反哺回平台**，是连接产品与客户的"胶水"（教材表述，"首创"缺乏官方定名依据）。
- 它有**三层含义**：岗位 / 团队 / 方法论。
