...
- **章节地图：** 第 3 章讲了"怎么干活"的方法论，本章确立**由谁来执行**：Echo（主责判断）负责前半（需求→施工图，Ch5/6），Delta（主责施工）负责后半（实现→Demo，Ch7–12），Ch16 全团队收口。

> **一句话记住本章：** Echo 主责判断、Delta 主责施工；人能判断、AI 能执行。**角色是"战位"不是"标签"**——战位随阶段迁移，个人能力按 T 型培养。
...
> **承接第 3 章 3.2：** 第 3 章已用"Echo 主责判断、Delta 主责施工、Engineering 负责工程保障"的职能视角画过三角色（图 3-2）。本节换一个视角——**战场比喻**（尖刀 / 政委 / 保阵地），并给出三者更完整的对照。两个视角是同一件事的两面。

一次 FDE 交付由一个"特种作战单元"承担。前线作战单元划分为**三角色**：
...
> **承接第 3 章 3.2：** 第 3 章已给 Echo 一句话定位（主责判断、出《解决方案框架》）。本节给出全量深度——职责、时间分配、能力画像、与客户经理的本质区别。
...
> **承接第 3 章 3.2：** 第 3 章已给 Delta 一句话定位（主责施工、把施工图实现为 Demo 并回注）。本节给出全量深度——职责、时间分配、能力画像、与"只会写码"的本质区别。
...
> [!CAUTION]
> **Delta ≠ "只会写码的程序员"。** 要能理解业务约束、参与技术可行性判断，并**主动做能力回注**——不做回注，Delta 就退化成外包（呼应"卖人力 vs 卖能力"）。
...
> 由此引出本书的**两种训练模式**：
> - **判断型实操（实操一、实操五）关键判断不用 AI**——AI 只能辅助整理/润色/扮演红队，判断力替不了；
> - **AI 施工品类（实操二、三、四）用 opencode + gstack**——训练施工力，让 coding agent 跑、人在 checkpoint 把关。
>
> 两者缺一不可——**"人会判断、AI 会执行"，是 FDE 交付的核心引擎，也是本书要练成的两种能力。**

**连续统一的团队：分工解耦，但同一坐标系协作。** Echo 与 Delta 不是"上下游两个部门"，而是同一个作战单元里**分工不同、目标一致**的队友（呼应 4.1"分工解耦 × 战斗力整合"）。**LLM 能力金字塔就是双方共用的坐标系**：Echo 负责"选层"——用业务判据决定该用提示词、RAG 还是 Agent（方向）；Delta 负责"建层"——用 AI Coding 把选定的层实现出来（落地）。金字塔既是分工的**分界线**（谁干什么一目了然），又是协作的**契合点**（Echo 说"第 2 层 RAG"，Delta 就知道要建"检索 + 引用"管线）——**选层与建层，是同一张图的两半。**

---

## 4.6 四阶段中的角色协作

不同阶段，谁主导、谁配合是不同的。下面这张矩阵告诉你"在项目每个阶段，Echo/Delta 各干什么"：
...
> **重要前提：** 本演练判断的是**战位倾向**，不是人格测验；结果只决定"谁主导哪一步"，不代表你只能干一种活。
...
> **这是战位倾向判断，不是人格测验。** FDE 是"T 型人才"——横线（Echo/Delta 两种能力都要宽）尽量铺，竖线（某一项要深）要有立足点。本测试只决定"谁主导哪一步"，不代表你只能干一种。
...
- **把"AI 施工"当成"跳过判断"。** 让 coding agent 一把梭、不设 checkpoint，或用 AI 堆方案代替自己思考——分工哲学要求"人能判断、AI 能执行"，不是相反。
