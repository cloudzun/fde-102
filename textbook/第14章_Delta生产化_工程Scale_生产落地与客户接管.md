...
> **本章定位：** Delta 侧生产落地单元，处在四阶段交付的 **Build 收口 + Scale 阶段**（坐标见第 3 章 3.3，角色分工见第 4 章 4.6）：承接 Prototype 阶段产出的 Demo，Delta 在此完成**生产评估、自运营交接、能力回注**三类职责。它不系统教授通用 DevOps 或 SRE，而是训练 FDE 如何把 Demo **接入客户真实数据、系统、流程和责任边界**，完成生产准入、客户接管与能力回注。以客户现场"最后一公里"为主线，通用运维内容只作为生产准入证据出现。
>
...
>
> **Delta 在 Scale 阶段的三项职责（对照第 4 章 4.6 矩阵）：** 1. **生产评估**——真实环境持续验证假设与指标；2. **自运营交接**——运行手册、接管演练、FDE 撤出；3. **能力回注**——工程机制与业务本体的第二场景验证。Echo 在 Scale 负责业务价值与组织采纳，Engineering 承接平台级运维深水区。
>
...
FDE 生产准入检查收敛为**八项 FDE 生产准入条件**（Gate 结果要能喂给第 16 章作 Go/No-Go 输入。对照第 3 章 3.3.2 Stage Gate 总纲——**本表是 Build Gate 的工程侧展开**，判断"能否上线交给客户"；涉及"公网 → 本地/微调"回填的组，第 13 章 13.4.4 的"切换 Gate 五项"作为本表**数据接入**与**发布回退**两项的补充证据；撤出条件再喂给 **Scale Gate**，判断"能否宣告交付完成"）：
