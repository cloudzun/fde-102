...
全章地图是五个转换——每一节都是其中一个转换的展开：**转换 1–3 完成 Build**（把原型嵌入真实业务，14.1.1–14.1.3；其中贯穿的"责任"假设核验见 14.2），**转换 4–5 属于 Scale 阶段**（14.1.4–14.1.5，对应 Delta 在 Scale 的"自运营交接"与"能力回注"两项职责；"生产评估"贯穿 14.2 与 14.5）：
...
**FDE 判断：** 谁启动/停止服务、谁处理失败请求、谁维护知识与规则、谁批准配置变更、谁解释指标变化、谁承担最终责任（六类现场问题，对应的客户侧 owner、操作项与撤出条件见 14.8）。
...
**数据假设。** 核验：数据是否真实存在；字段含义是否一致；数据质量能否支撑当前方案；是否及时更新；是否可在目标**安全域**内使用——这涉及数据分类分级与"数据不出域"红线（参考《数据安全法》确立的数据分类分级保护制度，[中伦解读](https://www.zhonglun.com/research/articles/8527.html)、[数据安全法简评](http://www.zz-iplaw.com.cn/CN/10441-11332.aspx)，教材提炼）；以及原型测试集是否代表真实业务分布。最常被跳过的是最后一条：原型测试集是自己造的，指标必然"好看"，真实分布一换数字就要重测。
...
**业务（流程）假设。** 核验：业务规则是否有正式口径；人工兜底是否有人承接；原有流程是否允许改变；业务人员是否接受新的职责；验收指标是否仍被业务方认可。**这一假设最容易被技术视角漏掉，却最影响能不能上线**——Echo 参与确认（呼应第 5 章干系人）。
...
FDE 生产准入检查收敛为**八项 FDE 生产准入条件**（教材提炼；Gate 结果要能喂给第 16 章作 Go/No-Go 输入。对照第 3 章 3.3.2 Stage Gate 总纲——**本表是 Build Gate 的工程侧展开**，判断"能否上线交给客户"；涉及"公网 → 本地/微调"回填的组，第 13 章 13.4.4 的"切换 Gate 五项"作为本表**数据接入**与**发布回退**两项的补充证据；14.8 的撤出条件再喂给 **Scale Gate**，判断"能否宣告交付完成"）：
...
> 配置、日志、重试、权限、回滚、监控仍然重要，但本章中它们是**生产准入证据**，不是课程主体。深水区实现（熔断器实现、监控平台搭建、分布式 Trace、Kubernetes、高可用集群、完整 SLO 方法、RTO/RPO 深入、供应链清单）**不在本书正文范围**，正文只留判断、验收与协作边界。**这些深水区若客户没有现成平台，由 Engineering（第 3 章 3.2.3）或客户平台团队承接，Delta 只定义准入条件与协作接口，不自行建设一套。**
...
对象的属性描述业务世界有什么；关系描述对象如何关联；动作和函数描述业务可以做什么；权限与治理约束谁可以执行。**本书借鉴这种方法（教材提炼），不要求使用特定平台**（完整"本体论"知识见第 15 章 15.4.3）。
...
> [!CAUTION]
> **不得把一张对象关系图直接称为平台能力。** 未完成数据、动作、权限和第二场景验证，只能叫"概念模型"（对应 Gate 的"回注准备"维）。三级命名与第 15 章 15.4.3 的"概念模型 → 操作型业务本体"一致。

---

## 14.10 贯穿实操：把一个 Demo 接入模拟客户环境

> 本实验不要求搭建完整生产基础设施，而是模拟 FDE 把 Demo 接入客户现场的全过程。**默认对象：第 8 章诉求分类器**（业务简单、依赖少、便于观察接入与接管）；进阶组可换第 12 章路由工作流，或换第 10 章政策 RAG 问答——选 RAG 时，把下方"模拟客户环境应提供"中的"模拟工单接口"替换为"政策文档更新与索引重建"、把"一次字段变更"替换为"一次政策文件版本更新"。
...
**环节五 · 完成 FDE 生产准入 Gate 评审**：按八项 Gate 逐项给出现场证据、未关闭风险、是否允许进入下一阶段、责任人与条件（14.5）。
...
| 案例 | 场景 | 关键教训 | 出处 |
|---|---|---|---|
| 案例一 | 部署即接入 | 部署到服务器 ≠ 接入客户业务 | 14.1.2 |
| 案例二 | 工单没分派出去 | 模型指标正确 ≠ 业务闭环完成 | 14.1.3 |
| 案例三 | 转人工没有接收人 | 人工兜底必须进入真实流程 | 14.4 |
| 案例四 | 部门职责调整 | 变更优先走配置与业务模型 | 14.6 |
| 案例五 | 离不开原作者 | 客户能自运营才是 Scale 成功 | 14.1.4 |
| 案例六 | 复制代码当复用 | 代码可复用 ≠ 业务语义可复用 | 14.1.5 |
...
- **Delta 在 Scale 的三项职责**：生产评估（14.2 / 14.5；含"开发者运行 → 客户团队运营"的运营侧核验，14.1.4）、自运营交接（14.8）、能力回注（14.9）——Echo 管业务侧（第 15 章），Engineering 管运维深水区（14.7）。
...
- **生产保障是准入证据不是课程主体**：配置、日志、重试、权限、回滚、监控只讲判断与验收，深水区不在本书正文范围（14.7）。
- **自运营与撤出**：六类 owner、接管演练、"客户依赖下降 = 完成标志"、撤出九项条件（14.8）。
- **回注**：工程能力 + 业务语义两类；概念模型 → 可执行（操作型）业务本体 → 已验证回注能力，第二场景验证（14.9）。
