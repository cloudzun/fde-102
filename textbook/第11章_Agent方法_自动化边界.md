...
> **前置：** 第 7 章（四层级金字塔 7.2）、第 8 章（一次调用 = 分类）、第 9 章（检索 + 生成 = RAG）。
...
>
> **提前说清三个词（完整辨析见 11.3.4）：** 本书严格区分 **工作流（Workflow，流程写死）**、**智能体工作流（Agentic Workflow，路径预编排、节点由 LLM 决策）**、**智能体（Agent，自主决策循环，路径不可穷举）**——工单三路分流这类路由按严格定义属于 Agentic Workflow，广义业界也常称其为 Agent，本书采用教材口径（见 11.3.4）。
...
```
*图 11-2：六种模式按厂商原始框架分两类；本书教学在此基础上增加 Agentic Workflow 中间层（11.3.4）*

### 11.3.5 工单分级为什么用 Routing 就够

工单分级是 Routing 的**典型应用场景**（业界运用普遍）：任务天然分几档（如咨询 / 跨部门 / 敏感），每档处理逻辑固定。**用 Routing 就够，不需要 Orchestrator-Workers 那种会"自我分解任务"的动态编排**——不为固定分档的任务搭一个最贵最慢的复杂智能体，这是 FDE"从最简单开始"的判断（呼应 11.2，且口径上它是 **Agentic Workflow**，见 11.3.4）。这类场景会在实操四中演练。

> **一个企业事实（呼应本章视野）：** 生产里**绝大多数"智能体系统"其实是"工作流 / Agentic Workflow + 局部智能体"的混合**——固定流程由工作流承担，少数需要判断/调工具的节点放一个小智能体，而不是整套都是几十个自主智能体来回奔跑。11.10 的银行反洗钱场景会完整演示这种混合形态。
...
- **简单场景不必上**：单个工具、单次接入，直接函数调用即可——从最简单开始（11.3.5）。

**与 agent 解决方案的关联与作用。**
- **工具层与决策层解耦**：决策循环（11.1 / 11.3）负责"下一步做什么"，MCP 负责"工具怎么接入"——换工具、加工具不碰 agent 逻辑；
- **与 11.5.1 LangGraph 配合**：LangGraph 管"决策图"（State / Node / interrupt），MCP 管"工具接入"——一个 agent 方案里两者各司其职；
- **承接 11.6 工具设计**：工具定义得好（11.6.1）+ 接入标准化（MCP）→ 工具库成为可复用资产；
- **能力回注落点**：工具接入标准化后，同一套工具能被不同模型、不同智能体框架复用，是能力回注（第 3 章四步法）在工具层的落点。
...
> [!CAUTION]
> **工具失败时"假装成功"是智能体工程最常见的翻车。** 兜底顺序是**失败 → 有限重试 → 降级（只读 / 简化）→ 转人工**，永远不要越过"转人工"这道线——尤其敏感路径（呼应 11.7.3 的"宁可多升级"与 11.8 三层护栏）。路由工作流用固定转人工分支即可；**受控 Agent（实操四B）**才把 `interrupt()`、持久化 `thread_id` 与工具超时/重试/降级纳入必做。
...
人机回环（Human-in-the-Loop）指**关键/敏感决策路径必须能暂停、等待人工确认**——智能体只能"建议"不能"拍板"。它是第 4 章"人能判断、AI 能执行"在智能体侧的工程化：**决策循环（11.1）负责跑，人在预设的暂停点接住关键决策**。
...
**三个工程要点（把 HITL 做扎实）：**
1. **暂停点选在哪**：影响真金白银 / 安全 / 合规 / 对外动作的路径，必须设暂停点（对应 11.5.1 的 interrupt()、路由工作流的固定"转人工"分支）；**其他路径不要处处暂停**——否则人工疲于确认，反而放松警惕；
...
> **注意（呼应 11.8.2 的"人类监督漏检"）：** HITL 不是免死金牌——人也会漏、会盲批。关键路径要**双人复核或抽检**，并记录"谁在什么时间批准了什么"（审计），让人工确认本身可被复盘。

> **与 agent 方案的关系：** HITL 是 agent 方案里的"人机回环"组件——`interrupt()`（11.5.1）提供暂停机制、固定"转人工"分支（路由工作流）提供兜底路径、`request_human_review`（实操四B）提供带上下文的确认请求；**有没有 HITL、HITL 放在哪，是方案设计时要写死的事，不是模型"自觉"。**
...
2. **出错代价多大？** 影响真金白银 / 安全 / 合规 / 对外承诺 → 必须留人工确认环（对应 11.8.3）；代价可逆、可回滚 → 可自动 + 事后审计；
3. **能否设计兜底？** 拿不准就异常返回、能兜底到人工 → 就不怕自动；无法兜底（一步错全盘错）→ 保守留人。

> **三问的用法：** 逐条过，任何一问指向"留人"，就按 11.8.3 设暂停点；三问全指向"可自动"，也仍要留审计（对应 11.8.1 的输出审查层）——"可自动"不等于"无人看"。

### 11.9.3 边界写进 SPEC，并落到工单分级

**边界要写进 SPEC（可验收）：** 自动化边界不是口头约定，要写死成规格，作为第 12 章实操四 DoD 的检查点（呼应 7.4 gstack 纪律）：
...
对路由工作流：咨询件自动化（模式固定、风险低）、跨部门件半自动（AI 分析 + 人可介入）、敏感件**必须转人工**（政务安全底线；**给定验证集召回率 100%，生产目标为零漏判**）。路由工作流用固定"转人工"分支即可，真正的中断—恢复由受控 Agent 实现（呼应 11.5.1）。而当你把同样的三问放到反洗钱场景（11.10），得到的边界几乎一模一样——**好的自动化边界判断，跨行业是通用的。**
...
| 红线 | 在此场景的落地 |
| :--- | :--- |
| **数据不出域** | 金融数据隔离部署在内部网段，模型走私有化（呼应第 13 章 13.2 本地部署） |
...
- **把自动化边界当纯技术问题甩给 AI。** "敏感件要不要转人工"是业务 / 安全决策，必须你拍板（对照 11.9）。
- **敏感件漏判（假阴性）。** 政务与金融场景假阴性代价远高于假阳性——验收口径"给定验证集召回率 100%"，宁多升级，不可漏（对照 11.7）。
- **工具定义随意、调用无工程化。** 描述不清、参数晦涩、无超时/重试/幂等/审计/降级，智能体准确率与稳定性直接崩（对照 11.6）。
- **工具失败时装成功。** 失败应走"重试→降级→转人工"，不能越级硬撑（对照 11.6.3）。
- **无兜底。** LLM 返回异常、解析失败时默认自动处理，会绕过人工安全线——应兜底到"转人工"（对照 11.7、11.8）。
- **把智能体当"万能自动机"。** 生产里智能体大多是"工作流 + 局部智能体 + 人机回环"的混合，不是几百个自主智能体（对照 11.3.5、11.10.3）。

> [!IMPORTANT]
> **红线本处落地：敏感件零漏判。** 本章的知识（假阴性代价、自动化边界、人机回环、"给定验证集召回率 100%"口径）就是第 12 章实操四"宁可多升级、不可漏"的理论依据，也是导读页四条红线本处闭环——且这套判断在银行反洗钱场景同样成立。
...
- **挑战（迁移练习）：** 选一个你熟悉的行业场景（如保险理赔、供应链异常、企业客服工单），设计一个"带人机回环的智能体"：说明用工作流还是框架编排、三路分流怎么分（并按 11.3.6 判定它的"预定义/动态 × 单轮/闭环"）、四条红线如何落地（零漏判写进验证集口径）、工具五件套与三层护栏怎么落、能力回注怎么落。
