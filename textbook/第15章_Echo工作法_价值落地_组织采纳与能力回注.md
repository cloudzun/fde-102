...
## 15.9 通用性：从西岭总结回顾到"复盘真实项目"

> **本章方法不只用于第 16 章对西岭项目做总结回顾**——它是一套**通用的 Echo 业务判断方法**，等价地用于对**本人真实项目**做拆解、复盘与决策。"复盘真实项目"（无编号可选实验，手册见《实验手册》`labs/migration-review-real-project.md`）用的就是同一套骨架：先拆解 → 再复盘 → 再决策汇报，不做新功能、不加新方法。

**同一套方法换到真实项目上，八个问题完全复用（每一条都要有证据）：**

1. 这个项目当前处于哪个阶段（Discovery / Prototype / Build / Scale）？
2. 它"验证"了什么——是功能可行，还是业务价值成立，还是组织愿意采用？
3. 还缺哪些证据？按状态分类：**已实测 / 客户确认 / 当前假设 / 待获取**——别把假设当结论；
4. 组织阻力在哪一块（15.4 门槛 · 动机 · 信任）？对应的动作是什么？
5. 值不值得继续？用 15.3 五口径算价值区间，结论是 Go / Conditional Go / Continue Pilot / No-Go 的哪一种？
6. 哪些能力值得回注（工程机制进 14.9、业务语义候选进 15.7）？哪些只是本项目的定制？
7. 数据与合规边界是什么（数据不出域、来源可追溯、责任归属）？
8. 需要决策层批准什么（资源、试点范围、本地部署立项）？

> **判断"方法真的拿走了"的标准：** 换一个你完全没跟过的项目，你仍然能回答这八个问题，并且答出的不是猜测而是证据——西岭主线是"别人设计好的练习"，真实项目没有标准答案，这正是检验"迁移"的地方（联动：16 章可选出口与导读"复盘真实项目"）。
