...
flowchart TB
    ...
    ch12["第 12 章·路由器 Agent<br/>三路分流 · 敏感件零漏判"]
    ch8 --> ch11
    ch9 --> ch11
    ch11 --> ent
    ent --> ch12
...
> **对实操四：** 它是用 LangGraph 把"判断 → 查部门职责库 → 生成意见 → 决定转人工"编排成一张图；`interrupt()` 正好承载"敏感件暂停转人工"这个必须有人的节点（配合 `thread_id` 持久化，实现真正的"暂停—恢复"，而非一次性输出）。
...
> [!CAUTION] **工具失败时"假装成功"是智能体工程最常见的翻车。** 兜底顺序是**失败 → 有限重试 → 降级（只读 / 简化）→ 转人工**，永远不要越过"转人工"这道线——尤其敏感路径（呼应 11.6.3 的"宁可多升级"与 11.9 三层护栏）。第 12 章实操四要求把 `interrupt()`、持久化 `thread_id` 与工具超时/重试/降级纳入必做。
...
对第 12 章：咨询件自动化（模式固定、风险低）、跨部门件半自动（AI 分析 + 人可介入）、敏感件**必须转人工**（政务安全底线；**给定验证集召回率 100%，生产目标为零漏判**）。这套边界判断，就是你要在实操四 SPEC 里**写死**的决策（呼应 7.6 gstack 的 SPEC 纪律），并把"敏感件暂停—人工确认—恢复"用 `interrupt()` + `thread_id` 真正实现（呼应 11.7.1）。而当你把同样的三问放到反洗钱场景（11.5），得到的边界几乎一模一样——**好的自动化边界判断，跨行业是通用的。**
...
- 见在线全书 https://www.cloudzun.com/fde-course/（第 15 章 AI FDE 落地 · Echo 共识与对齐）——AI 能力与判断力边界如何对齐的扩展。
- 见在线全书 https://www.cloudzun.com/fde-course/（第 16 章 AI FDE 落地 · Delta 技术与场景）——国产智能体平台（Dify / Coze / 阿里云百炼）等的落地选型。
- 下一章：**本书第 12 章实操四 · 工单智能分级路由 Agent**——把 Routing（Agentic Workflow）模式 + 敏感件零漏判当场做出来。