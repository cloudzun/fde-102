# 附加迁移实验：用 FDE 方法复盘真实项目（学员版）

> **实验性质：** 课程结束后的**无编号、独立、可选**迁移挑战。它不属于"实操六"，不是实操五的一部分，也不影响基础结业。
> **一句话目标：** 把你在西岭课程主线里学会的 FDE 理念、流程和判断框架，迁移到你自己的**真实项目**上，完成拆解、复盘与决策层汇报。
> **角色：** 仅使用 **Echo**（靠想：挖真问题、拆场景、判断价值）和 **Delta**（靠做：验证技术、评估落地、守住边界）。不新增任何第三种 FDE 角色。
> **建议时长：** 课前准备 + 约 3 小时（含汇报与质询）。
> **完成前提：** 已学完五个主线实操（及 Lab4B 如已完成）。真实项目可按需匿名化。
> **核心原则：框架不变，项目换成你自己的工作。** 西岭是给全班统一的练习；这份附加实验，是检验你能否把方法带回真实世界。

---

## 实验概述

五个主线实操用**西岭市民服务平台**这个统一虚构案例，完成了系统性的方法训练。它承担：统一客户背景、统一需求材料、统一 A/B/C 三个场景、统一分类/RAG/Routing Workflow/Lab4B 的能力递进、统一 Echo/Delta 分工、统一验收口径、统一实操五收口。

这份附加迁移实验，是课程主干之外的**可选挑战**：

- **没有合适真实项目的学员**：完成主线即可达到课程基本要求，不参加本实验不影响结业。
- **有合适真实项目的学员**：选择本人一个真实业务项目，撤下"西岭"这个统一壳，把同一套方法用一遍。

```mermaid
flowchart TB
    Main[西岭课程主干<br/>统一材料与统一验收] --> Complete[完成五个主线实操<br/>及 Lab4B]
    Complete --> Optional{是否有合适的真实项目}
    Optional -->|没有或不便公开| Finish[完成主线即可结业]
    Optional -->|有且可匿名化| Transfer[附加迁移实验<br/>拆解·复盘·汇报]
...
西岭统一案例是"别人设计好的练习"——需求、干系人、坑都预先埋好，讲师知道答案。真实项目**没有标准答案**：你的客户、你的问题、你的证据、你的判断，只有你自己能负责。这正是迁移的价值所在。

> **AI 可以用，但关键判断必须是你（Echo/Delta）自己下。** AI 可以帮你整理材料、归类证据、扮演红队质询；但"客户真正的问题是什么""这个风险能不能接受""值不值得继续投"——这些决定了项目成败，AI 替不了你。

---

## 本轮核心学习点：把方法从"案例"迁移到"真实世界"
...
| 主线哪里学 | 迁移到哪里 |
|---|---|
| 实操一：Echo 读客户材料 | 整理你的真实项目材料和原始需求 |
| 四类失败模式 | 检查真实项目的数据、业务、合规、ROI 风险 |
| 四层决策链 | 识别你客户的决策层/操作层/技术层/监管层 |
| 能力金字塔选型 | 判断你的项目该用 Prompt/分类/RAG/Workflow/Agent/微调/不用 AI |
| 实操二/三/四：三个单点 MVP | 判断你的项目需不需要这几个能力 |
| Lab4B：协同 Agent | 判断你的项目是否真的需要动态选工具、追问、人工暂停 |
| 实操五：双视角收口 | 由 Echo/Delta 共同给出下一阶段决策和汇报 |

> **记住这条主线：** 先用单点 MVP 学会能力积木，再用 Lab4B 学会动态协同，最后由全团队判断能否交付、怎样接管、什么值得回注。这份迁移实验，是把"最后这一步"放到你自己的项目上再做一次。
...
## 1. Echo 重走 Discovery（约 60 分钟）——靠想，断清"问题是什么"

> 这一部分由 **Echo** 主导。目标不是"描述你的项目现状"，而是**像实操一那样，从一堆模糊材料里挖出真正值得做的问题**。
...
## 2. Delta 重审 Prototype、Build 与 Scale（约 60 分钟）——靠做，验清"能不能落地"
...
## 3. Echo + Delta 双视角收口（约 30 分钟）——合起来，形成团队判断
...
### 项目需要补做原型时
如决定实际开发或改造原型，沿用 gstack 八环节：office-hours → spec → autoplan → build → review → qa → ship → retro。
...
4. 不增加 Echo、Delta 之外的第三角色；
5. 不要求重新开发完整系统；
6. 不把已有功能介绍当作 FDE 项目复盘；
7. 不把当前假设写成已验证结论；
8. 不编造 ROI、用户反馈和测试结果；
9. 不泄露客户数据、凭据、源码或商业秘密；
10. 不重新发明与实操一、实操五不一致的分析框架；
...
> **换掉西岭案例后，你是否仍能以 Echo 找对问题，以 Delta 验证和落地，并由 Echo 与 Delta 共同完成能力回注和决策汇报。**

西岭用统一案例把方法教给你；这份实验，是把方法交还给你自己。